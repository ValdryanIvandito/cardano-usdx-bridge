...
**_xReserve_** adalah infrastruktur interoperabilitas yang memungkinkan para pengembang _blockchain_ untuk menerbitkan _stablecoin_ berbasis USDC di jaringan mereka sendiri. Sistem ini didukung oleh pengesahan terprogram (_programmatic attestations_) dan kontrak cerdas yang diimplementasikan oleh Circle yang menyimpan USDC sebagai cadangan di _source blockchain_ seperti Ethereum. _stablecoin_ berbasis USDC dan diterbitkan melalui _xReserve_ dapat saling berinteraksi serta dengan jaringan USDC yang lebih luas.

## Manfaat Utama

_xReserve_ menawarkan berbagai manfaat bagi jaringan _blockchain_, para pengembang, dan pengguna akhir:

- Likuiditas hari pertama: Token yang diterbitkan menggunakan _xReserve_ terhubung dengan jaringan USDC yang lebih luas, sehingga pengguna dapat dengan mudah memindahkan token antar _blockchain_.

- Interoperabilitas: Pengguna dapat melakukan transfer 1:1 antara _stablecoin_ berbasis USDC dan USDC.

- Diimplementasikan oleh Circle: _xReserve_ menyimpan cadangan USDC dalam kontrak cerdas yang diimplementasikan oleh Circle.
...
1. Pengguna mendepositkan USDC dari aplikasi _wallet_ mereka ke kontrak cerdas _xReserve_ di _source blockchain_.
...
6. Kontrak token di _remote blockchain_ mendepositkan _stablecoin_ yang baru dicetak ke aplikasi wallet pengguna di _remote blockchain_ tersebut.
...
1. Pengguna mengajukan pembakaran _stablecoin_ berbasis USDC di _remote blockchain_ serta penarikan USDC di _destination blockchain_.
...
5. _xReserve_ memverifikasi pembakaran tersebut dan menerbitkan pengesahan penarikan.
